...
use crate::{
    error::P2PError,
    events::{emit_event, EVT_ORDER_EXPIRED, OrderExpiredEvent},
...
pub fn process(
    ...
    let [caller, order_acc, market_acc, _owner, owner_token, vault, _token_program, ..] =
        accounts
    else {
    ...
    let (owner_key, refund_amount) = {
        ...
        if !order.is_expired(clock.unix_timestamp) {
            return Err(P2PError::OrderNotExpired.into());
        }
    ...
    caller.set_lamports(caller.lamports().checked_add(rent).ok_or(P2PError::Overflow)?);
    order_acc.set_lamports(0);
    order_acc.close()?;

    let ts = Clock::get()?.unix_timestamp;
    emit_event!(OrderExpiredEvent {
        discriminator: EVT_ORDER_EXPIRED,
